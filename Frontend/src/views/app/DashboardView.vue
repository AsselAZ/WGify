<template>
  <div class="min-h-screen">
    <AppNavbar title="Dashboard" subtitle="Willkommen zurück, Assel!" />
        ...
        <div class="rounded-xl border border-border bg-card p-6">
          ...
          <div class="space-y-3">
            <div
              ...
              :key="expense.id"
              :class="['flex items-center justify-between rounded-lg px-4 py-3', index % 3 === 0 ? 'bg-secondary/50' : index % 3 === 1 ? 'bg-accent/30' : 'bg-purple/10']"
            >
              <div>
                <p class="font-medium">{{ expense.title }}</p>
                <p class="text-sm text-muted-foreground">{{ expense.paidBy }} - {{ formatDate(expense.date) }}</p>
              </div>
        ...
        <div class="rounded-xl border border-border bg-card p-6">
          ...
          <div class="space-y-3">
            <div
              ...
              :key="task.id"
              :class="['flex items-center justify-between rounded-lg px-4 py-3', index % 2 === 0 ? 'bg-accent/30' : 'bg-secondary/50']"
            >
              <div>
                <p class="font-medium">{{ task.title }}</p>
                <p class="text-sm text-muted-foreground">{{ task.assignedTo }} - Fällig: {{ formatDate(task.dueDate) }}</p>
              </div>
              <span class="inline-flex items-center rounded-full bg-warning/20 px-2.5 py-0.5 text-xs font-medium text-warning-foreground">Offen</span>
            </div>
          </div>
        </div>
      </div>
